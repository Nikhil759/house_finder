#!/usr/bin/env python3
"""
enrich_geocode_telegram.py — Geocode Telegram listings that have no coordinates.
...
Usage:
    python3 scripts/enrich_geocode_telegram.py
    python3 scripts/enrich_geocode_telegram.py --dry-run
    python3 scripts/enrich_geocode_telegram.py --limit 20
    python3 scripts/enrich_geocode_telegram.py --dry-run --limit 10
"""
...
parser = argparse.ArgumentParser(
    description="Geocode Telegram listings with null latitude"
)
...
def centroid_for_locality(locality: str) -> tuple[float, float] | None:
    ...
    if not locality:
        return None
    # Try direct lookup first (canonical name)
...
def main() -> None:
    ...
    cur.execute("SELECT COUNT(*) FROM listings WHERE source='telegram';")
    total_tg: int = cur.fetchone()[0]

    cur.execute("SELECT COUNT(*) FROM listings WHERE source='telegram' AND latitude IS NULL;")
    ...
    section("TELEGRAM GEOCODING ENRICHMENT")
    print(f"  Total telegram rows         : {total_tg:,}")
